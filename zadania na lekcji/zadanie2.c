...
int licz(char tab[10][20], int ktory)	//np. dla jednego calego rzedu ('ktory'ego)
{
		...
		for(i=0;i<20;i++)
		{
			if(islower(tab[ktory][i])&&tab[ktory][i]!='\0'&&tab[ktory][i]!='\n'&&tab[ktory][i]!=' '&&tab[ktory][i]!='0') suma++;
			printf("%i SUMA=%i\n",i+1,suma);
		}
...
int main()
{
	...
	for(j=0;j<10;j++)	//wypelniam zerami, bo czesto sie zdarzalo ze wystepowala jakas losowa litera na pozycji 18...
		{
			for(k=0;k<20;k++)
				tab[j][k]='0';
		}
